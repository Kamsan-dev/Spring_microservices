...
CREATE TABLE IF NOT EXISTS users (
    user_id BIGSERIAL PRIMARY_KEY,
    user_public_id VARCHAR(40) NOT NULL,
    ...
    qr_code_image_uri TEXT NOT NULL,
    ...
    uptaded_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_users_email UNIQUE (email),
    CONSTRAINT uq_users_user_public_id UNIQUE (user_public_id)
);

CREATE TABLE IF NOT EXISTS roles (
    role_id BIGSERIAL PRIMARY_KEY,
    role_public_id VARCHAR(40) NOT NULL,
...
CREATE TABLE IF NOT EXISTS user_roles (
    user_role_id BIGSERIAL PRIMARY_KEY,
...
CREATE TABLE IF NOT EXISTS credentials (
    credential_id BIGSERIAL PRIMARY_KEY,
    credential_public_id VARCHAR(40) NOT NULL,
    user_id BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS account_tokens (
    account_tokens_id BIGSERIAL PRIMARY_KEY,
...
CREATE TABLE IF NOT EXISTS password_tokens (
    password_tokens_id BIGSERIAL PRIMARY_KEY,
    token VARCHAR(40) NOT NULL,
    user_id BIGINT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    uptaded_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS devices (
    device_id BIGSERIAL PRIMARY_KEY,
    user_id BIGINT NOT NULL,
...
CREATE OR REPLACE PROCEDURE create_user(
    ...
    --- queries
        INSERT INTO users (user_public_id, first_name, last_name, email, username, member_id) VALUES (
            p_public_id, p_first_name, p_last_name, p_email, p_username, p_member_id) RETURNING user_id INTO v_user_id;
        ...
        INSERT INTO user_roles (user_id, role_id) VALUES(
            v_user_id,
            (SELECT FROM roles (roles.role_id) WHERE roles.name = 'USER')
        );
